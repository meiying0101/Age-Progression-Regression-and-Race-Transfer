...
class FaceAging(object):
    def __init__(self,
                 ...
                 size_batch=16,  # mini-batch size for training and testing, must be square of an integer
                 ...
                 num_categories=4,  # number of categories (race segments) in the training dataset
                 ...
                 #save_dir='./save0102',  # path to save 
    ...
    def train(self,
        ...
        self.loss_EG = self.EG_loss + 0.00001 * self.G_img_loss + 0.001 * self.E_z_loss + 0.001 * self.tv_loss # slightly increase the params  
        ...
        self.EG_optimizer = tf.train.AdamOptimizer(              #GradientDescentOptimizer
        ...
        for i, label in enumerate(sample_files):
            ...
            temp = int(str(sample_files[i]).split('/')[-1].split('_')[2].split('.')[0])
        ...
        for epoch in range(num_epochs):
            ...
            for ind_batch in range(num_batches):
                ...
                for i, label in enumerate(batch_files):
                    ...
                    temp = int(str(batch_files[i]).split('/')[-1].split('_')[2].split('.')[0])
                    if temp ==0:
                        label = 0
                    elif temp ==1:
                        label = 3
                ...
                print("\tEG_err=%.4f\tEz=%.4f\tDz=%.4f\tDzp=%.4f\tGi=%.4f\tDi=%.4f\tDiG=%.4f" % (EG_err, Ez_err, Dz_err, Dzp_err, Gi_err, Di_err, DiG_err))
    ...
    def test(self, images, gender, name):
        ...
        print("query_labels.shape[0]: ", query_labels.shape[0])
        ...
        print("query_gender: ", query_gender.shape)
        z, G = self.session.run(
            [self.z, self.G],
            feed_dict={
